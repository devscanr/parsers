...
from extractors.extractors import parse_phones, parse_emails
...
def describe_parse_phones() -> None:
  ...
  def parses_newlines() -> None:
    assert parse_phones("+49\n30\n12345678") == ["+493012345678"]
  ...
  def parses_urls() -> None:
    ...
    assert parse_phones("whatsapp: https://wa.me/+13096314912") == ["+13096314912"]
    assert parse_phones("https://wa.me/+2347032434912") == ["+2347032434912"]
    assert parse_phones("https://wa.me/+2347032434912?someparam") == ["+2347032434912"]
  ...
  def parses_prefixed_set6() -> None:
    assert parse_phones("QQ:308066400 Zzz:+1.2086141414 phone：+86.13516521121") == ["+8613516521121"]
  ...
  def parses_prefixed_set9() -> None:
    assert parse_phones("Phone numbers: +1 (443) 216-1111") == ["+14432161111"]  # number(s)
    assert parse_phones("📞 + 91 - 7680896811") == ["+917680896811"]  # space after +

  def parses_prefixed_set10() -> None:
    # Messengers
    assert parse_phones("WhatsApp: +36202687810") == ["+36202687810"]
    assert parse_phones("Viber: +36202687810") == ["+36202687810"]
    assert parse_phones("Telegram: +36202687810") == ["+36202687810"]
    assert parse_phones("Signal: +36202687810") == ["+36202687810"]
    assert parse_phones("Phone/Viber/WhatsApp: +36202687812") == ["+36202687812"]
    assert parse_phones("WhatsApp / Telegram / Signal: +1(647) 373 1111") == ["+16473731111"]
